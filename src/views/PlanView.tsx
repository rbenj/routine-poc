...
const PlanView = () => {
  ...
  useEffect(() => {
    ...
    return () => {
      if (newIntervalId !== undefined) {
        // If this run actually created an interval
  ...
  const formatDuration = (totalSeconds: number): string => {
    ...
    if (seconds > 0 || minutes === 0) {
      // Show seconds if it's the only unit or if minutes are also present
        ...
        >
          <div className="task-header">
            ...
            {((isExpanded && !isCurrentlyExecutingTask) ||
              (!isExpanded && !isCurrentlyExecutingTask)) && <span>{taskItem.durationSec}s</span>}
          ...
          {isExpanded && isCurrentlyExecutingTask && (
            <div className="task-controls">
              ...
              <button
                ...
                disabled={currentTaskIndex === plan.tasks.length - 1}
              >
                Skip
              </button>
            </div>
    ...
    return (
      ...
      >
        <div className="task-header">
          ...
          timeRemaining !== null &&
          task.fields?.find((f) => f.type.includes('timer')) ? (
            <div className="timer">{formatTime(timeRemaining)}</div>
          ) : (
            task.estimatedTimeSec > 0 && (
              <span className="estimated-time-header">
                Est: {formatDuration(task.estimatedTimeSec)}
              </span>
            )
          )}
        </div>
              ...
              return (
                <div className="field" key={field.name}>
                  ...
                  {field.type === 'timer_down' || field.type === 'timer_up' ? (
                    <span className="field-value timer-field">
                    ...
                    <div className="field-controls">
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          updateFieldValue(task.name, taskCount, field, -1);
                        }}
  ...
  return (
    <div className="plan-view">
          ...
          <div className="plan-progress-details">
            ...
            <div className="plan-percent-complete">
              ...
              {initialPlanDuration > 0
                ? (
                    ((initialPlanDuration - currentPlanTimeRemaining) / initialPlanDuration) *
                    100
                  ).toFixed(0)
                : 0}
              %
      ...
      <div className="task-list">{plan.tasks.map((task, index) => renderTask(task, index))}</div>
    </div>
  );
};
